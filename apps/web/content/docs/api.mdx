...
## `GET /v1/records/:id/conversation-snapshots`

`GET /v1/records/:id/conversation-snapshots` returns observed X reply snapshots
...
curl -H "Authorization: Bearer $ARGUS_API_TOKEN" \
  "$ARGUS_URL/v1/records/$RECORD_ID/conversation-snapshots"
```
...
source links. The CLI command is the easiest client:
...
`GET /v1/primitives/web/search?q=...` calls only the configured SearXNG origin
and forces `format=json`. It uses the same authentication, body, redirect,
timeout, and rate bounds and also writes nothing.
