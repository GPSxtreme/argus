...
Use Ubuntu 22.04 and Ubuntu 24.04, or Debian 12 and Debian 13, on Linux amd64/x64 or arm64/aarch64. The host needs a sudo-capable account, outbound HTTPS, Docker Engine with the Compose plugin and daemon access, at least 5 GiB free disk, and 1 GiB memory. Choose 2 GiB when onboarding managed SearXNG. Onboarding also verifies that the selected API port is free before it writes an instance.
