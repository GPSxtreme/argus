...
For a SQLite VPS, stop Argus before copying the database and its `-wal` and `-shm` companions from `argus-data`, or use SQLite's online backup tooling. Restore all three together. Argus copies discovered SQLite database files and `state.json` into timestamped `/opt/argus/backups/<version>-<timestamp>/` directories before a non-noop update, but that update backup is not a substitute for a separately tested backup.
