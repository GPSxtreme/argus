...
`GET /v1/records/:id/conversation-snapshots` with the configured API token.

**Meaning:** Reply tracking is opt-in, ends at `maxTrackingHours`, observes at
most 10 pages or 500 replies per refresh, and retains at most `maxPerPost` in
the configured order. `complete` describes the bounded collection run, not all
replies on X.
