...
Each recovery starts with a non-mutating inspection. Run a `--dry-run` plan before every CLI mutation that supports it, then add `--yes` only after reviewing the current plan. Stable error codes below are the CLI or doctor codes to retain when escalating.
...
**Recover:** Start Docker, install the Compose plugin, or grant the operating user daemon access as the reported prerequisite requires. Then rerun `argus onboard --dry-run --json` before an onboarding mutation, or rerun `argus doctor --json` for an existing instance.
...
**Meaning:** `UPDATE_ROLLBACK_UNAVAILABLE` means there is no valid persisted update backup or verified rollback selection. `UPDATE_ROLLBACK_INCOMPATIBLE` means the backup and requested signed release cannot be safely paired.

**Recover:** Do not delete recovery files. If the dry-run returns a valid rollback plan, run `argus update --rollback --yes --json`; otherwise restore from the separately retained SQLite or PostgreSQL backup and verify with `argus status --json` and `argus doctor --json`.
