...
Check the deployment and diagnostics. Each JSON response uses the stable
versioned CLI envelope; a healthy result has `ok: true`.
...
immediate job has not completed yet; inspect `argus doctor --json` before
changing the host.
