...
Transient source primitives require a configured API token. The X primitive
accepts only GET/HEAD under normalized `/2/` paths on the configured FxEmbed
origin; the Web primitive exposes only JSON search on the configured SearXNG
origin. Callers cannot choose an upstream host. Both paths cap redirects,
request time, body size, and per-token/source rate, strip caller headers, and
never persist their response.
