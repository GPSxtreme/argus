...
The runtime refuses to bind a non-loopback API host without `api.token`. Loopback includes `127.0.0.1`, `::1`, `::ffff:127.0.0.1`, and `localhost`. Docker Compose publishes the selected API port, so an Internet-facing VPS still needs an operator-controlled firewall, reverse proxy, or private network policy. Prefer a loopback bind and expose only a terminating proxy when remote access is required.
